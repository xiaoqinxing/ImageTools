...
#         # scale the data
#         data = out_min + (data - in_min) * (out_max -
...
#                 data2list = data.reshape(-1,
#                                          data.shape[1]*data.shape[2]).tolist()
#                 # write in the file
...
class helpers:
    ...
    def sobel_prewitt_direction_label(self, gradient_magnitude, theta, threshold=0):
        ...
        direction_label = np.zeros(
            np.shape(gradient_magnitude), dtype=np.float32)
    ...
    def edge_wise_median(self, kernel_size, edge_location):
        ...
        no_of_pixel_pad = math.floor(
            kernel_size / 2)   # number of pixels to pad
        ...
        for i in range(no_of_pixel_pad, height + no_of_pixel_pad):
            for j in range(no_of_pixel_pad, width + no_of_pixel_pad):
                if (edge_location[i, j] == 1):
                    ...
                    output[i - no_of_pixel_pad, j -
                           no_of_pixel_pad] = data[i, j]
    ...
    def nonuniform_quantization(self):
        ...
        mask = (self.data > (3./4.) * (max_val - min_val)
                ) & (self.data <= (7./8.) * (max_val - min_val))
        output[mask] = 2.

        mask = (self.data > (1./2.) * (max_val - min_val)
                ) & (self.data <= (3./4.) * (max_val - min_val))
        output[mask] = 1.
...
class synthetic_image_generate:
    ...
    def create_lens_shading_correction_images(self, dark_current=0, flat_max=65535, flat_min=0, clip_range=[0, 65535]):
        ...
        flat_field_image = np.empty(
            (self.height, self.width), dtype=np.float32)

        center_pixel_pos = [self.height/2, self.width/2]
        max_distance = distance_euclid(
            center_pixel_pos, [self.height, self.width])

        for i in range(0, self.height):
            for j in range(0, self.width):
                flat_field_image[i, j] = (
                    max_distance - distance_euclid(center_pixel_pos, [i, j])) / max_distance
                flat_field_image[i, j] = flat_min + \
                    flat_field_image[i, j] * (flat_max - flat_min)

        dark_current_image = np.clip(
            dark_current_image, clip_range[0], clip_range[1])
        flat_field_image = np.clip(
            flat_field_image, clip_range[0], clip_range[1])
    ...
    def sobel(self, kernel_size):
        ...
        if (kernel_size > 3):
            ...
            for i in range(0, n):
                ...
                Sx = (
                    1./16.) * signal.convolve2d(np.dot([[1.], [2.], [1.]], [[1., 2., 1.]]), Sx)
...
class color_conversion:
    ...
    def rgb2ycc(self, rule="bt601"):
        ...
        output[:, :, 0] = kr * self.data[:, :, 0] + \
            ...
            kb * self.data[:, :, 2]
        output[:, :, 1] = 0.5 * \
            ((self.data[:, :, 2] - output[:, :, 0]) / (1 - kb))
        output[:, :, 2] = 0.5 * \
            ((self.data[:, :, 0] - output[:, :, 0]) / (1 - kr))
    ...
    def ycc2rgb(self, rule="bt601"):
        ...
        output = np.empty(np.shape(self.data), dtype=np.float32)
        output[:, :, 0] = 2. * self.data[:, :, 2] * \
            (1 - kr) + self.data[:, :, 0]
        output[:, :, 2] = 2. * self.data[:, :, 1] * \
            (1 - kb) + self.data[:, :, 0]
        output[:, :, 1] = (self.data[:, :, 0] - kr *
                           output[:, :, 0] - kb * output[:, :, 2]) / kg
    ...
    def rgb2xyz(self, color_space="srgb", clip_range=[0, 65535]):
        ...
        if (color_space == "srgb"):
            ...
            output = np.empty(np.shape(self.data), dtype=np.float32)
            output[:, :, 0] = data[:, :, 0] * 0.4124 + \
                data[:, :, 1] * 0.3576 + data[:, :, 2] * 0.1805
            output[:, :, 1] = data[:, :, 0] * 0.2126 + \
                data[:, :, 1] * 0.7152 + data[:, :, 2] * 0.0722
            output[:, :, 2] = data[:, :, 0] * 0.0193 + \
                data[:, :, 1] * 0.1192 + data[:, :, 2] * 0.9505
            ...
            # matrix multiplication
            output = np.empty(np.shape(self.data), dtype=np.float32)
            output[:, :, 0] = data[:, :, 0] * 0.5767309 + \
                data[:, :, 1] * 0.1855540 + data[:, :, 2] * 0.1881852
            output[:, :, 1] = data[:, :, 0] * 0.2973769 + \
                data[:, :, 1] * 0.6273491 + data[:, :, 2] * 0.0752741
            output[:, :, 2] = data[:, :, 0] * 0.0270343 + \
                data[:, :, 1] * 0.0706872 + data[:, :, 2] * 0.9911085
    ...
    def xyz2rgb(self, color_space="srgb", clip_range=[0, 65535]):
        ...
        if (color_space == "srgb"):
            ...
            # matrix multiplication
            output[:, :, 0] = self.data[:, :, 0] * 3.2406 + \
                self.data[:, :, 1] * -1.5372 + self.data[:, :, 2] * -0.4986
            output[:, :, 1] = self.data[:, :, 0] * -0.9689 + \
                self.data[:, :, 1] * 1.8758 + self.data[:, :, 2] * 0.0415
            output[:, :, 2] = self.data[:, :, 0] * 0.0557 + \
                self.data[:, :, 1] * -0.2040 + self.data[:, :, 2] * 1.0570
            ...
            # matrix multiplication
            output[:, :, 0] = self.data[:, :, 0] * 2.0413690 + \
                self.data[:, :, 1] * -0.5649464 + \
                self.data[:, :, 2] * -0.3446944
            output[:, :, 1] = self.data[:, :, 0] * -0.9692660 + \
                self.data[:, :, 1] * 1.8760108 + self.data[:, :, 2] * 0.0415560
            output[:, :, 2] = self.data[:, :, 0] * 0.0134474 + \
                self.data[:, :, 1] * -0.1183897 + \
                self.data[:, :, 2] * 1.0154096

            # gamma to retain nonlinearity
            output = helpers(
                output * clip_range[1]).gamma_adobe_rgb_1998(clip_range)
    ...
    def lab2lch(self):
        ...
        output[:, :, 1] = np.power(
            np.power(self.data[:, :, 1], 2) + np.power(self.data[:, :, 2], 2), 0.5)
        output[:, :, 2] = np.arctan2(
            self.data[:, :, 2], self.data[:, :, 1]) * 180 / np.pi
    ...
    def lch2lab(self):
        ...
        output[:, :, 0] = self.data[:, :, 0]  # L transfers directly
        output[:, :, 1] = np.multiply(
            np.cos(self.data[:, :, 2] * np.pi / 180), self.data[:, :, 1])
        output[:, :, 2] = np.multiply(
            np.sin(self.data[:, :, 2] * np.pi / 180), self.data[:, :, 1])
    ...
    def sobel(self, kernel_size=3, output_type="all", threshold=0., clip_range=[0, 65535]):
        ...
        if np.ndim(self.data) > 2:
            ...
            for dimension_idx in range(0, np.shape(self.data)[2]):
                Gx[:, :, dimension_idx] = signal.convolve2d(
                    self.data[:, :, dimension_idx], Sx, mode="same", boundary="symm")
                Gy[:, :, dimension_idx] = signal.convolve2d(
                    self.data[:, :, dimension_idx], Sy, mode="same", boundary="symm")
        ...
        if np.ndim(self.data > 2):
            for i in range(0, np.shape(self.data)[2]):
                direction_label[:, :, i] = helpers().sobel_prewitt_direction_label(
                    G[:, :, i], theta[:, :, i], threshold)
        else:
