...
DOWNLOAD_INSTALL_PACK = 'ImageTools.exe'
...
def download_file(srcUrl, localFile):
    """
    func: 下载一个大文件
    """
    info('开始下载 %s --->>>  %s' % (srcUrl, localFile))
    startTime = time.time()
...
def update_now():
    download_file(REMOTE_INSTALL_PACK, DOWNLOAD_INSTALL_PACK)
    system(DOWNLOAD_INSTALL_PACK)
    exit(0)
...
class CheckUpdate(QDialog):
    ...
    def __init__(self, parent):
        ...
        self.ui.ok.clicked.connect(update_now)
    ...
    def set_autoupdate_param(self, p):
        self.autoupdate = (p != 0)
        info('autoupdate' + str(self.autoupdate))
