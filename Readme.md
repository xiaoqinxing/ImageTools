...
目前涉及到的算法：镜头景深计算，图像抖动程度的量化，数字水印，ISP算法（黑电平，去马赛克，白平衡，色彩校正，局部对比度增强，坏点矫正）
...
---
## （六）PQtools转代码工具
这是一款能够将PQtools生成的xml转换成头文件的工具，方便把参数写入代码。
### 界面
1. 右侧是设置
2. 左侧是生成的头文件预览

### 使用方法
1. 点击打开xml文件按钮，设置需要转换的xml文件路径
2. 点击输出文件路径，设置转换后的头文件路径
3. 下面的设置可以用默认的，也可以自定义
4. 缩进的空格数：可以控制代码的缩进
5. 设置一行最大字符数：到了一定的字数会自动换行
6. 产品ID：可以自定义输出的变量名称
7. 需要去除的元素：默认是rb，xml中有部分元素和结构体对不上，可以去除这部分元素
8. 需要输出的结构体：告诉工具需要过滤输出哪些结构体，默认的几个结构体都验证过没有问题
9. 需要包含的头文件：用默认就好
10. 自定义结构体：可以用来解决结构体里面含有指针的问题，含有指针不好直接初始化，需要转换成数组，默认提供了3DNR的自定义结构体
11. 需要替换的结构体：可以把原生的结构体替换成自定义的结构体，一般与上面联用
12. 版权声明：定义头文件起始的版权说明，宏定义%AUTHOR%会自动转换成作者名，%NOW_TINE%会自动转换成当前时间
13. 是否需要加const：可以选择生成的变量前面加不加const，加了const就不能修改里面的值了
14. 设置完毕后，点击生成，当进度条到了100%的时候，就生成完毕，在输出文件的路径下会自动生成头文件，当然也可以在左侧的预览窗口进行复制粘贴
