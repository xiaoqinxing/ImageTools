# ImageTools
- *作者：[李钦星](https://www.qinxing.xyz/)* 
- *版本：1.4* 

这是一款图像相关的工具软件，扩展性好。目前集成有镜头计算器、抖动测试工具、看图工具等功能。可以方便的进行后期算法的验证（热更新、直观的图像显示和分析）。退出后会自动保存上次打开的窗口，以及窗口里的参数。

目前涉及到的算法：镜头景深计算，图像抖动程度的量化，数字水印，ISP算法（黑电平，坏点矫正，暗影矫正，去马赛克，白平衡，色彩校正，gamma，局部对比度增强，色度空间转换，对比度亮度调整，小波降噪WNR，锐化）
...
- [ImageTools](#imagetools)
  ...
  - [五. 视频对比工具](#五-视频对比工具)
    - [界面](#界面-2)
    - [使用方法](#使用方法-2)
  - [六. PQtools转代码工具](#六-pqtools转代码工具)
    - [背景](#背景)
    - [使用方法](#使用方法-3)
    - [基础设置](#基础设置)
    - [自定义设置（可以用默认的，或者自定义）](#自定义设置可以用默认的或者自定义)
  - [七. 代码拓展与编译](#七-代码拓展与编译)
    - [1. 扩展方法](#1-扩展方法)
    - [2. vscode配置方法](#2-vscode配置方法)
      - [安装依赖](#安装依赖)
      - [UI修改](#ui修改)
    - [3. 打包方法](#3-打包方法)
...
位置：工具->水印
1. 导入水印图片
2. 选择水印类型：半透明水印（可以看到）、空域隐形水印（看不到）
3. 如果是半透明水印，可以调整水印大小和水印的透明度
4. 如果是空域隐形水印，可以调整水印的大小和二值化的阈值
5. 点击`制作带水印的图片`，可以制作出最后带水印的图片。在点击这个按钮之前显示的都是预览效果
6. 点击`解析空域隐形水印`，可以解析出由本软件制作出来的空域水印。这个功能是独立的，并非需要先制作水印才能解析。
...
6. **算法调试**：如果需要进行算法的调试，修改相关ISP算法代码之后，点击`算法热更新`，可以重新加载ISP相关算法，不需要重新启动程序。如果报错，弹出的窗口会显示错误信息。
...
### 基础设置
1. **输入设置**：将pqtools调试的参数另存为xml文件。点击`打开xml文件`按钮，设置成需要转换的pqtools参数文件。
...
4. **需要去除的元素**：默认是`rb`，pqtools生成的文件中有部分元素和代码中的结构体定义对不上，需要去除这部分元素
5. **需要输出的结构体**：pqtools生成的xml文件里包含了所有的参数，参数太多了，其中有很多用不到的，这就需要进行一个删减和过滤，把需要输出的结构体列举出来，一行一个。例如`hiISP_SHARPEN_ATTR_S`
6. **需要包含的头文件**：自定义该头文件需要包含哪些头文件，一般用默认就好
7. **自定义结构体**：可以用来解决结构体里面含有指针的问题，含有指针的结构体不好直接通过赋值初始化，需要先转换成数组，然后把数组转换成指针。默认提供了3DNR的自定义结构体`customNRX_PARAM_AUTO_V1_S`
...
## 七. 代码拓展与编译
### 1. 扩展方法
1. tools类中增加工具的使用方式和界面，可以直接继承SubWindow类，参考pqtools_to_code.py
...
### 3. 打包方法
运行buildexe.bat, 注意需要安装打包库`pyinstaller`和打包软件`inno setup compiler`